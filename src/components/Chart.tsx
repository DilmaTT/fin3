...
export const Chart = ({ isMobileMode, charts, onCreateChart, onDeleteChart, onEditChart, onPlayChart }: ChartProps) => {
  ...
  const handleConfirmDelete = () => {
    if (deleteCandidateId) {
      onDeleteChart(deleteCandidateId);
  ...
  return (
    <>
                ...
                <Card key={chart.id} className="transition-all hover:bg-muted/50">
                  <CardContent className="p-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="destructive" size="icon" onClick={(e) => { e.stopPropagation(); setDeleteCandidateId(chart.id); }}>
      ...
      <AlertDialog open={deleteCandidateId !== null} onOpenChange={(isOpen) => !isOpen && setDeleteCandidateId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Вы уверены?</AlertDialogTitle>
            <AlertDialogDescription>
              Это действие невозможно отменить. Это приведет к необратимому удалению чарта.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
